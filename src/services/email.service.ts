...
export class EmailService {
  ...
  async send(data: Product[]) {
    ...
    try {
      ...
      const mailOptions = {
        from: this.configService.get<string>("MAIL_FROM"),
        to: this.configService.get<string>("MAIL_TO_ERROR"),
        subject: `Errore articoli rilevati da ${data[0].userCode} il ${data[0].date.split(",")[0]}`,
        text: "Il servizio di scan ha prodotto un errore",
      };
